...
# modifitied by [Ali Hassanzadeh] [https://github.com/iPmartNetwork]
...
_check_and_set_venv(){
    ...
    if [ ! -d "$VIRTUAL_ENV" ]; then
        ...
        { $pythonExecutable -m venv "$VIRTUAL_ENV"; } >> ./log/install.txt 2>&1
    fi
...
_checkPythonVersion(){
	...
	if [ "$version_pass" == "1" ]; then 
		...
		printf "[WGDashboard] WGDashboard requires Python 3.10, 3.11, or 3.12. Halting install now.\n"
		kill $TOP_PID
	fi
}
