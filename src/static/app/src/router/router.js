...
const router = createRouter({
	...
	routes: [
		...
		},
		{
			...
			meta: {
				requiresAuth: true,
				title: "Welcome to iPWG Dashboard",
				hideTopNav: true
			},
		},
		{
...
router.beforeEach(async (to, from, next) => {
	...
	if (to.meta.title){
		if (to.params.id){
			document.title = to.params.id + " | iPWG Dashboard";
		}else{
			document.title = to.meta.title + " | iPWG Dashboard";
		}
	}else{
		document.title = "iPWG Dashboard"
	}
	...
	if (to.meta.requiresAuth){
		if (!dashboardConfigurationStore.getActiveCrossServer()){
			if (await checkAuth()){
				...
				next("/signin")
				dashboardConfigurationStore.newMessage("iPWG Dashboard", "Sign in session ended, please sign in again", "warning")
			}
		}else{
			await dashboardConfigurationStore.getConfiguration()
			if (!wireguardConfigurationsStore.Configurations && to.name !== "Configuration List"){
