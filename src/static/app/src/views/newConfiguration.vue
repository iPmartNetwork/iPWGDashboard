	...
	methods: {
		...
		readFile(e){
			...
			reader.onload = (evt) => {
				...
				if (appliedFields > 0){
					this.dashboardStore.newMessage("iPWG Dashboard", `Parse successful! Updated ${appliedFields} field(s)`, "success")
				}else {
					this.dashboardStore.newMessage("iPWG Dashboard", `Parse failed`, "danger")
				}
			};
