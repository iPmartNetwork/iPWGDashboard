...
UPDATE = None
app = Flask("iPWG Dashboard", template_folder=os.path.abspath("./static/app/dist"))
...
class WireguardConfiguration:
        ...
        if name is not None:
            ...
            if "Backup" not in data.keys():
                ...
                with open(self.configPath, "w+") as configFile:
                    ...
                    print(f"[iPWG Dashboard] Configuration file {self.configPath} created")
                self.__initPeersList()

        if not os.path.exists(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup')):
            os.mkdir(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup'))
        
        print(f"[iPWG Dashboard] Initialized Configuration: {name}")    
        if self.getAutostartStatus() and not self.getStatus() and startup:
            self.toggleConfiguration()
            print(f"[iPWG Dashboard] Autostart Configuration: {name}")
            ...
            with open(self.configPath, 'r') as configFile:
                ...
                try:
                    ...
                    if __name__ == '__main__':
                        print(f"[iPWG Dashboard] {self.Name} Error: {str(e)}")
        else:
            self.Peers.clear()
    ...
    def getPeersTransfer(self):
        ...
        try:
            ...
            print(f"[iPWG Dashboard] {self.Name} Error: {str(e)} {str(e.__traceback__)}")
    ...
    def backupConfigurationFile(self) -> tuple[bool, dict[str, str]]:
        ...
        shutil.copy(
            self.configPath,
            os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', f'{self.Name}_{time}.conf')
        )
        with open(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', f'{self.Name}_{time}.sql'), 'w+') as f:
    ...
    def getBackups(self, databaseContent: bool = False) -> list[dict[str: str, str: str, str: str]]:
        ...
        directory = os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup')
        ...
        for f, ct in files:
            if RegexMatch(f"^({self.Name})_(.*)\\.(conf)$", f):
                ...
                d = {
                    "filename": f,
                    "backupDate": date,
                    "content": open(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', f), 'r').read()
                }
                if f.replace(".conf", ".sql") in list(os.listdir(directory)):
                    d['database'] = True
                    if databaseContent:
                        d['databaseContent'] = open(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', f.replace(".conf", ".sql")), 'r').read()
                backups.append(d)
    ...
    def restoreBackup(self, backupFileName: str) -> bool:
        ...
        if self.Status:
            self.toggleConfiguration()
        target = os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backupFileName)
        targetSQL = os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backupFileName.replace(".conf", ".sql"))
    ...
    def deleteBackup(self, backupFileName: str) -> bool:
        ...
        if backupFileName not in backups:
            return False
        try:
            os.remove(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backupFileName))
        except Exception as e:
            return False
        return True
    ...
    def downloadBackup(self, backupFileName: str) -> tuple[bool, str] | tuple[bool, None]:
        ...
        with ZipFile(os.path.join('download', zip), 'w') as zipF:
            zipF.write(
                os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backup[0]['filename']),
                os.path.basename(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backup[0]['filename']))
            )
            if backup[0]['database']:
                zipF.write(
                    os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backup[0]['filename'].replace('.conf', '.sql')),
                    os.path.basename(os.path.join(self.__getProtocolPath(), 'iPWG Dashboard_Backup', backup[0]['filename'].replace('.conf', '.sql')))
                )
    ...
    def getNumberOfAvailableIP(self):
            ...
            for pip in peerAllowedIP:
                ...
                if len(ppip) == 2:
                    try:
                        ...
                        print(f"[iPWG Dashboard] Error: {self.Name} peer {p.id} have invalid ip")
        ...
        for ca in configurationAddresses:
            ...
            try:
                ...
                print(e)
                print(f"[iPWG Dashboard] Error: Failed to parse IP address {ca} from {self.Name}")
...
def sqlSelect(statement: str, paramters: tuple = ()) -> sqlite3.Cursor:
    ...
    try:
        ...
        print("[iPWG Dashboard] SQLite Error:" + str(error) + " | Statement: " + statement)
    return result
...
def API_addWireguardConfiguration():
    ...
    if "Backup" in data.keys():
        ...
        if (os.path.exists(os.path.join(path['wg'], 'iPWG Dashboard_Backup', data["Backup"])) and
                os.path.exists(os.path.join(path['wg'], 'iPWG Dashboard_Backup', data["Backup"].replace('.conf', '.sql')))):
            protocol = "wg"
        elif (os.path.exists(os.path.join(path['awg'], 'iPWG Dashboard_Backup', data["Backup"])) and
              os.path.exists(os.path.join(path['awg'], 'iPWG Dashboard_Backup', data["Backup"].replace('.conf', '.sql')))):
            protocol = "awg"
        else:
            return ResponseObject(False, "Backup does not exist")
        
        shutil.copy(
            os.path.join(path[protocol], 'iPWG Dashboard_Backup', data["Backup"]),
...
def API_getAllWireguardConfigurationBackup():
    ...
    for protocol in ProtocolsEnabled():
        directory = os.path.join(DashboardConfig.GetConfig("Server", f"{protocol}_conf_path")[1], 'iPWG Dashboard_Backup')
        files = [(file, os.path.getctime(os.path.join(directory, file)))
                 for file in os.listdir(directory) if os.path.isfile(os.path.join(directory, file))]
        files.sort(key=lambda x: x[1], reverse=True)
            ...
            if RegexMatch(r"^(.*)_(.*)\.(conf)$", f):
                ...
                if name not in existingConfiguration:
                    ...
                    d = {
                        ...
                        "filename": f,
                        "backupDate": date,
                        "content": open(os.path.join(DashboardConfig.GetConfig("Server", f"{protocol}_conf_path")[1], 'iPWG Dashboard_Backup', f), 'r').read()
                    }
                    if f.replace(".conf", ".sql") in list(os.listdir(directory)):
                        d['database'] = True
                        d['databaseContent'] = open(os.path.join(DashboardConfig.GetConfig("Server", f"{protocol}_conf_path")[1], 'iPWG Dashboard_Backup', f.replace(".conf", ".sql")), 'r').read()
...
def API_getDashboardAPIKeys():
    ...
    return ResponseObject(False, "iPWG Dashboard API Keys function is disabled")
...
def API_getDashboardUpdate():
    ...
    try:
        r = req.urlopen("https://api.github.com/repos/donaldzou/iPWG Dashboard/releases/latest", timeout=5).read()
...
def peerInformationBackgroundThread():
    ...
    print(f"[iPWG Dashboard] Background Thread #1 Started", flush=True)
    time.sleep(10)
    while True:
            ...
            for c in WireguardConfigurations.values():
                if c.getStatus():
                    try:
                        ...
                        print(f"[iPWG Dashboard] Background Thread #1 Error: {str(e)}", flush=True)
        time.sleep(10)

def peerJobScheduleBackgroundThread():
    with app.app_context():
        print(f"[iPWG Dashboard] Background Thread #2 Started", flush=True)
        time.sleep(10)
        while True:
            AllPeerJobs.runJob()
            time.sleep(180)
