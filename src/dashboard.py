...
@app.get(f'{APP_PREFIX}/api/generateKeyPair')
def API_generateKeyPair():
    try:
        ...
        log(f"Error generating key pair: {e}", level="error", source="API_generateKeyPair")
        return ResponseObject(False, message=f"Failed to generate key pair: {e}", status_code=500)

@app.get(f'{APP_PREFIX}/api/getPeerScheduleJobs')
def API_getPeerScheduleJobs():
    ...
    configuration = WireguardConfigurations[config_name]
    found_peer, _ = configuration.searchPeer(peer_id)
    if not found_peer:
        return ResponseObject(False, "Peer not found in the specified configuration", status_code=404)

    jobs = AllPeerJobs.searchJob(Configuration=config_name, Peer=peer_id)
    return ResponseObject(True, data=[job.toJson() for job in jobs])
...
def API_savePeerScheduleJob():
    ...
    if not f:
        return ResponseObject(False, "Peer does not exist in this configuration", status_code=404)
    
    # Validate JobID for new jobs if it's expected to be generated by client or ensure it's empty
    # For existing jobs, JobID must be present
    job_id = job.get('JobID')
    if not job_id and job.get('CreationDate'): # Likely an attempt to update without JobID
        return ResponseObject(False, "JobID is required for updating an existing job.", status_code=400)
    
    if not job_id: # New job, generate JobID if not provided (or let DB handle it if it's auto-increment/default)
        job['JobID'] = str(uuid.uuid4()) # Ensure JobID is a string for PeerJob constructor
        job['CreationDate'] = None # Let DB handle creation date for new jobs

    s, p = AllPeerJobs.saveJob(PeerJob(
        job['JobID'], job['Configuration'], job['Peer'], job['Field'], job['Operator'], str(job['Value']), # Ensure Value is string
        job['CreationDate'], job.get('ExpireDate'), job['Action'])) # Use .get for optional ExpireDate
    if s:
        # p here is a list containing the saved/updated job object
        return ResponseObject(True, data=p[0].toJson() if p and len(p) > 0 else None)
    return ResponseObject(s, message=str(p))
...
def API_deletePeerScheduleJob():
    ...
    job_to_delete = PeerJob(
        job['JobID'], job['Configuration'], job['Peer'], 
        job.get('Field', ''), job.get('Operator', ''), str(job.get('Value', '')), # Provide defaults for fields not strictly needed for deletion by JobID
        job.get('CreationDate', datetime.now().strftime('%Y-%m-%d %H:%M:%S')), # CreationDate is needed for deleteJob logic
        job.get('ExpireDate'), job.get('Action', '')
    )
    s, p = AllPeerJobs.deleteJob(job_to_delete)
    if s:
        return ResponseObject(True, message="Job deleted successfully.")
    return ResponseObject(s, message=str(p))
